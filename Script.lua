...
Notify("Project Silver", "Full Script Loaded")
...
local mt_supported = success and mt

if not mt_supported then
    Notify("Project Silver", "Инжектор не поддерживает защиту (Metatables).")
end

local old_index
local old_newindex

if mt_supported then
    setreadonly(mt, false)
    old_index = mt.__index
    old_newindex = mt.__newindex

    mt.__index = newcclosure(function(self, key)
        if not checkcaller() and AimSettings.HitboxEnabled and self:IsA("BasePart") and self.Name == "Head" and key == "Size" then
            return ORIGINAL_HEAD_SIZE
        end
...
local function IsSleeper(model)
    ...
    if lt then
        ...
        if rr and typeof(rr.CurrentAngle) == "number" and rr.CurrentAngle ~= 0 then
            return true
        end
    end
    return false
...
table_insert(UI.Connections, RunService.Heartbeat:Connect(function()
    ...
    if WorldSettings.Fullbright then
        local brightVal = WorldSettings.Intensity / 5
         Lighting.Ambient = Color3.new(brightVal, brightVal, brightVal)
         Lighting.OutdoorAmbient = Color3.new(brightVal, brightVal, brightVal)
        ...
        for _, obj in pairs_iter(workspace:GetChildren()) do
            if obj:IsA("Model") then
                if (obj:FindFirstChild("LowerTorso") or obj:FindFirstChild("HumanoidRootPart")) and obj.Name ~= LocalPlayer.Name then
                    table_insert(pList, obj)
                else
                    local type, part, col = identifyModel(obj)
                    if type then table_insert(oList, {model = obj, part = part, type = type, color = col}) end
                end
            end
        end
        ...
        for i = 1, #CachedPlayers do
            ...
            if head then
                if AimSettings.HitboxEnabled then
                    local jitter = AimSettings.HitboxSize + (math_random(-10, 10) / 100) 
                    head.Size = Vector3.new(jitter, jitter, jitter)
                    head.Transparency = 0.4 head.CanCollide = false
                elseif head.Size ~= ORIGINAL_HEAD_SIZE then
                    head.Size = ORIGINAL_HEAD_SIZE head.Transparency = ORIGINAL_TRANSPARENCY
                end
            end
        end
    end
...
table_insert(UI.Connections, RunService.RenderStepped:Connect(function()
    if env.Trident_SessionID ~= script_id then 
        for _, v in pairs_iter(UI.Elements) do pcall(function() v:Remove() end) end 
        return 
    end
    ...
    for i, name in ipairs(tabOrder) do
        ...
        if btn then
            ...
            btn.Position = menuPos + Vector2.new(20 + (i-1) * 100, 43)
            btn.Color = UI.CurrentTab == name and UI.Accent or Color3.fromRGB(130, 130, 140)
        end
    end
    ...
    for i, name in ipairs(subList) do
        ...
        sbtn.Visible, sbtn.Text = isVisible, name
        sbtn.Position = menuPos + Vector2.new(20 + (i-1)*120, 73)
        sbtn.Color = UI.CurrentSubTab == name and UI.Accent or Color3.fromRGB(80, 80, 90)
    end
    ...
    for id, elements in pairs_iter(UI.MenuElements) do
        ...
        if isCurrent then
            ...
            for i, el in ipairs(elements) do
                local col = (i <= mid) and 0 or 1
                local indexInCol = (i <= mid) and (i - 1) or (i - mid - 1)
                local startX = (col == 0) and 25 or 290
                local offY = 115 + (indexInCol * 35) + (UI.ScrollOffset[id] or 0)
                local show = isVisible and (offY >= 105 and offY <= 380)
                
                if el.type == "Toggle" then
                    el.bg.Position, el.out.Position, el.lbl.Position = menuPos+Vector2.new(startX, offY+3), menuPos+Vector2.new(startX, offY+3), menuPos+Vector2.new(startX+22, offY)
                    el.bg.Color = el.state and UI.Accent or Color3.fromRGB(30, 30, 40)
                ...
                elseif el.type == "Slider" then
                    ...
                    el.sFill.Size, el.sFill.Color = Vector2.new(220*((el.val-el.min)/(el.max-el.min)), 3), UI.Accent
                    el.valLbl.Text, el.valLbl.Position = string.format("%.1f", el.val), menuPos+Vector2.new(startX+220-el.valLbl.TextBounds.X, offY-5)
    ...
    FOVCircle.Position = mouse_loc
    FOVCircle.Color = AimSettings.Color
    FOVCircle.Filled = AimSettings.FilledFOV
    FOVCircle.Transparency = AimSettings.FilledFOV and (1 - AimSettings.FOVFillTransparency) or 1
    
    if CrosshairSettings.Enabled then
        if CrosshairSettings.Spinning then 
            CrosshairSettings.Rotation = CrosshairSettings.Rotation + (CrosshairSettings.SpinSpeed / 100) 
        end
        for i, line in ipairs(CrosshairLines) do
            ...
            line.From = mouse_loc + Vector2.new(math.cos(angle) * CrosshairSettings.Gap, math.sin(angle) * CrosshairSettings.Gap)
            line.To = mouse_loc + Vector2.new(math.cos(angle) * (CrosshairSettings.Gap + CrosshairSettings.Size), math.sin(angle) * (CrosshairSettings.Gap + CrosshairSettings.Size))
        end
    ...
    if hitDiff < HitmarkerSettings.Duration then
        ...
        local s, g = HitmarkerSettings.Size, 4
        local data = {
            {Vector2.new(-s, -s), Vector2.new(-g, -g)}, 
            {Vector2.new(s, -s), Vector2.new(g, -g)}, 
            {Vector2.new(-s, s), Vector2.new(-g, g)}, 
            {Vector2.new(s, s), Vector2.new(g, g)}
        }
        for i = 1, 4 do
            ...
            line.Visible, line.Transparency, line.Color, line.Thickness = true, alpha, HitmarkerSettings.Color, HitmarkerSettings.Thickness
            line.From = mouse_loc + data[i][1]
            line.To = mouse_loc + data[i][2]
        end
    else for _,l in pairs_iter(HitLines) do l.Visible = false end end

    for obj, drawings in pairs_iter(ESP_Storage) do if not obj or not obj.Parent then for _, d in pairs_iter(drawings) do d.Visible = false d:Remove() end ESP_Storage[obj] = nil end end
    for obj, drawings in pairs_iter(Ores_Storage) do if not obj or not obj.Parent then for _, d in pairs_iter(drawings) do d.Visible = false d:Remove() end Ores_Storage[obj] = nil end end
    ...
    for i = 1, #CachedPlayers do
        ...
        if root then
            ...
            if onScreen and env.ESP_Enabled and dist <= env.ESP_Dist and not IsSleeper(obj) then
                if not ESP_Storage[obj] then ESP_Storage[obj] = {Box = draw("Square", {Thickness = 1, Visible = false, ZIndex = 1}), Name = draw("Text", {Size = 13, Center = true, Visible = false, ZIndex = 1}), Dist = draw("Text", {Size = 11, Center = true, Visible = false, ZIndex = 1})} end
                local d = ESP_Storage[obj]
            ...
            elseif ESP_Storage[obj] then ESP_Storage[obj].Box.Visible = false ESP_Storage[obj].Name.Visible = false ESP_Storage[obj].Dist.Visible = false end
        end
    end
    ...
    for i = 1, #CachedOres do
        ...
        if enabled and ore.model and ore.model.Parent then
            ...
            if onScreen and dist < 1500 then
                if not Ores_Storage[ore.model] then Ores_Storage[ore.model] = {Name = draw("Text", {Size = 13, Center = true, Visible = false, ZIndex = 0})} end
    ...
    if AimSettings.Enabled and UserInputService:IsMouseButtonPressed(AimSettings.Key) then
        ...
        for i = 1, #CachedPlayers do
            local obj = CachedPlayers[i]
            local t = obj and obj.Parent and obj:FindFirstChild(AimSettings.TargetPart)
            if t and IsVisible(t) and not IsSleeper(obj) then
                local p, on = Camera:WorldToViewportPoint(t.Position)
                if on then
                    local mag = (Vector2.new(p.X, p.Y) - mouse_loc).Magnitude
                    if mag < short then short = mag target = p end
                end
            end
        end
        if target then
            local dist_factor = math.clamp(short / AimSettings.FOV, 0.1, 1)
            local adaptive_smooth = AimSettings.Smoothness * (1 + (1 - dist_factor))
            mousemoverel((target.X - mouse_loc.X) / adaptive_smooth, (target.Y - mouse_loc.Y) / adaptive_smooth) 
        end
    end
end))
...
table_insert(UI.Connections, UserInputService.InputChanged:Connect(function(input)
    if UI.Visible and input.UserInputType == Enum.UserInputType.MouseWheel then
        local m = UserInputService:GetMouseLocation()
        if m.X >= menuPos.X and m.X <= menuPos.X + menuSize.X and m.Y >= menuPos.Y and m.Y <= menuPos.Y + menuSize.Y then
            local id = UI.CurrentTab .. UI.CurrentSubTab
